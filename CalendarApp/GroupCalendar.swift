...
class GroupCalendar: NSObject {
    ...
    func createParse(title: String,callback: () -> Void) {
        ...
        calendarQuery.whereKey("password", equalTo: Calendar.sharedInstance.password)
        calendarQuery.getFirstObjectInBackgroundWithBlock { (object, notFind) -> Void in
            if notFind == nil {
                ...
                calendarObject["password"] = Calendar.sharedInstance.password
                ...
                calendarObject.saveInBackgroundWithBlock { (success, error) in
                    if success {
                        calendarQuery.getFirstObjectInBackgroundWithBlock { (object, error) -> Void in
                            if error == nil {
                                Calendar.sharedInstance.object_id = (object?.objectId)!
                                callback()
                            } else {
                            }
                        }
...
//        print(Calender.sharedInstance.object_id)
        ...
        dateQuery.whereKey("calendar_id", equalTo: Calendar.sharedInstance.object_id)
...
//                        print("タップされたdateの削除完了")
                ...
                dateObject["calendar_id"] = Calendar.sharedInstance.object_id
